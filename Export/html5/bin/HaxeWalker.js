...
var Main = function() {
	...
	this.HxWalker = new Walker(this.stage.stageWidth / 2,this.stage.stageHeight / 2);
	this.previousTime = openfl.Lib.getTimer();
	this.addEventListener(openfl.events.Event.ENTER_FRAME,$bind(this,this.loop));
};
$hxClasses["Main"] = Main;
Main.__name__ = ["Main"];
...
Walker.__super__ = openfl.display.Sprite;
Walker.prototype = $extend(openfl.display.Sprite.prototype,{
	step: function() {
		...
		case 6:
			...
			_g19;
			var _g20 = this;
			var _g110 = _g20.get_y();
			_g20.set_y(_g110 + 1);
			_g110;
			break;
		case 7:
			var _g21 = this;
			var _g111 = _g21.get_x();
			_g21.set_x(_g111 - 1);
			_g111;
			break;
		default:
			haxe.Log.trace("Not possible",{ fileName : "Walker.hx", lineNumber : 30, className : "Walker", methodName : "step"});
		}
		if(this.get_x() > openfl.Lib.current.stage.stageWidth) this.set_x(0);
		if(this.get_x() < 0) this.set_x(openfl.Lib.current.stage.stageWidth);
		if(this.get_y() > openfl.Lib.current.stage.stageHeight) this.set_y(0);
		if(this.get_y() < 0) this.set_y(openfl.Lib.current.stage.stageHeight);
	}
	,draw: function() {
		openfl.Lib.current.stage.get_graphics().beginFill(Math.floor(Math.random() * 16777215),1);
		openfl.Lib.current.stage.get_graphics().drawCircle(this.get_x(),this.get_y(),1);
		openfl.Lib.current.stage.get_graphics().endFill();
	}
	,__class__: Walker
});
var haxe = {};
...
//# sourceMappingURL=HaxeWalker.js.map